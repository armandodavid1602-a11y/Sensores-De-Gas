...
function onMessageArrived(message) {
    ...
    if (topic === TOPIC_VENTILADOR) {
        if (payload === 'VENTILADOR=ON') updateDeviceState(estadoVentilador, 'Encendido', 'text-green-400');
        else if (payload === 'VENTILADOR=OFF') updateDeviceState(estadoVentilador, 'Apagado', 'text-red-400');
    }

    // Ventana
    if (topic === TOPIC_VENTANA) {
        if (payload === 'VENTANA=ABRIR') updateDeviceState(ventanaEstado, 'Abierta', 'text-green-400');
        else if (payload === 'VENTANA=CERRAR') updateDeviceState(ventanaEstado, 'Cerrada', 'text-red-400');
    }

    // Alarma
    if (topic === TOPIC_ALARMA) {
        if (payload === 'ALARM=ON') updateDeviceState(alarmaEstado, 'Activada', 'text-green-400');
        else if (payload === 'ALARM=OFF') updateDeviceState(alarmaEstado, 'Desactivada', 'text-red-400');
    }
}
